...
from modules.agents.attention import RoleAwareAttention
from modules.agents.trust_estimator import TrustEstimator
...
class Communication(nn.Module):
    ...
    def aggregate(self, query, key, value, hidden_states, send_target, roles, role_embeddings, action_head_bank, comm_history=None, past_usefulness=None):
        bs = query.shape[0]
        
        # 1. Role Embeddings lookup
        roles_tensor = torch.tensor(roles, device=query.device) # [n_agents]
        role_embs = role_embeddings(roles_tensor).unsqueeze(0).repeat(bs, 1, 1) # [bs, n_agents, role_emb_dim]
        ...
        for agent_id in range(self.n_agents):
            ...
            q = action_head_bank(combined_projected, role_name)
