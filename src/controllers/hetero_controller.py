...
class HeteroMAC:
    def __init__(self, scheme, groups, args):
        ...
        self.crit_fun = torch.nn.CrossEntropyLoss().cuda() if args.use_cuda else torch.nn.CrossEntropyLoss()
    ...
    def forward(self, ep_batch, t, test_mode=False, counterfactual=False):
        ...
        # Ensure correct device allocation for eye tensor
        eye_tensor = torch.eye(self.n_agents, device=agent_inputs.device)
        send_target = torch.clamp(self.generate_send_target(send_prob) - eye_tensor, 0,
                                  1).int().detach()
    ...
    def _build_agents(self, input_shape):
        self.agent = HeteroAgent(input_shape, self.args)
