...
class EpisodeRunner:
    ...
    def run(self, test_mode=False):
        ...
        if getattr(self.args, "heterogeneous", False) and not test_mode:
            ...
            # Log role-wise trust scores
            trust_matrix = getattr(self.mac, "trust_scores", th.zeros(self.args.n_agents, self.args.n_agents))
            for i in range(self.args.n_agents):
                role_idx = self.mac.agent_roles[i]
                role_name = self.args.role_names[role_idx]
                agent_trust = trust_matrix[i].mean().item()
                self.role_metrics_logger.log_metric(f"trust_{role_name}", agent_trust, self.t_env)
                
            self.role_metrics_logger.log_metric("role_reward", episode_return, self.t_env)
            
            if self.t_env % 2000 == 0:
                self.role_metrics_logger.plot_metrics()
